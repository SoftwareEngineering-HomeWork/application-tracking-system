...
app.put("/:applicationId", async (req, res) => {
  try {
      ...
      const user = await Users.findById(userId);
      const applicationIndex = user.applications.findIndex(app => app.id === parseInt(applicationId));

      if (applicationIndex === -1) {
          return res.status(400).json({ error: "Application not found" });
      }

      user.applications[applicationIndex] = { ...user.applications[applicationIndex], ...requestData };
      await user.save();
